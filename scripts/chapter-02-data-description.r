#-------------------------------------------------------------------------------
#' chapter-02-data-description.r
#-------------------------------------------------------------------------------
#' jo dudding
#' April 2025
#' chapter 2 data and code
#-------------------------------------------------------------------------------
...
z_wt <- tibble(
  z = -2:2,
  z_band = factor(abs(z)),
  weight = mean(penn$weight) + z * sd(penn$weight),
  y = 0.3
)
...
penn |> 
  ...
  geom_line(data = z_wt, aes(weight, y), colour = plasma_pal[8]) +
  geom_point(data = z_wt, aes(weight, y, colour = z_band), 
    size = 6) +
  geom_text(data = z_wt, aes(weight, y, label = z),  
    colour = 'white', size = 4) +
  ...
  guides(y = 'none', fill = 'none', colour = 'none')
